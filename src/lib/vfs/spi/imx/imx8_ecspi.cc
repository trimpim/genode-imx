 ...
 */
Spi::Driver* Spi::initialize(Vfs::Env &vfs_env, const Xml_node &node) {
	auto config = Parse_ecspi_config(node);
	auto *driver = new (vfs_env.alloc()) Ecspi_driver { vfs_env.env(), config };
	return reinterpret_cast<Spi::Driver*>(driver);
}
