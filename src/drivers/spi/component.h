...
protected:
	...
	Attached_ram_dataspace    _tx_buffer;
	Attached_ram_dataspace    _rx_buffer;
	...
	:
		...
		_tx_buffer     { _ram_allocator, region_map, io_buffer_size },
		_rx_buffer     { _ram_allocator, region_map, io_buffer_size }
	{ }
};
...
public:
	...
	void bus_duplex_transfer(size_t buffer_size, uint16_t max_burst_size, bool byte_reordering) {
		...
		Driver::Bus_transaction bus_trxn {
			...
			.byte_reordering = byte_reordering,
			.tx_buffer       = _tx_buffer.local_addr<uint8_t>(),
			.rx_buffer       = _rx_buffer.local_addr<uint8_t>(),
			.buffer_size     = buffer_size,
		};
		_driver.transfer(bus_trxn);
	}

	Dataspace_capability tx_dataspace() { return _tx_buffer.cap(); }
	Dataspace_capability rx_dataspace() { return _rx_buffer.cap(); }

	void     settings(Settings settings) override { _settings = settings; }
	Settings settings() override { return _settings; }

	/* unused, RPC use bus_duplex_transfer */
	void duplex_transfer(Spi::Transfer const &) override {}
};
