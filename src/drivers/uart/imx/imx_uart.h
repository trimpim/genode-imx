 ...
 */
class Genode::Imx_uart: Platform::Device::Mmio
{
	public:
		...
		{
			uint32_t     clock_speed { 25000000 };
			uint32_t     baud_rate   { 115200 };
			Serial_mode  mode        { Serial_mode::rs232 };
		};
	...
	private:
		 ...
		 */
		struct Rxd : Register<0x00, 32>
		{
			struct Rx_data : Bitfield<0, 8> { };   /* receive data */
		};
		...
		struct Cr1 : Register<0x80, 32>
		{
			...
			struct At_dma_en  : Bitfield<2, 1> { };    /* aging DMA timer on */
			...
			struct Adbr     : Bitfield<14, 1> { };     /* enable baud-rate auto detect */
		...
		struct Cr3 : Register<0x88, 32>
		{
			...
			struct Dtrd_en    : Bitfield<3, 1> { };   /* data terminal ready delta IRQ on */
			...
			struct Dcd_en     : Bitfield<9, 1> { };   /* data carrier detect IRQ on */
			...
			struct Dtr_en     : Bitfield<13,1> { };   /* data terminal ready IRQ on */
			struct Dpec_ctrl  : Bitfield<14,2> { };   /* DTR/DSR IRQ edge control */
		...
		struct Cr4 : Register<0x8c, 32>
		{
			...
			struct IR_sc       : Bitfield<5, 1> { };   /* use UART ref clock for vote logic */
		...
		/*
		 * RS-485 Mode Control Register
		 */
		struct Mcr : Register<0xB8, 32>
		{
			struct Md_en   : Bitfield<0, 1>       /* Multidrop for RS-485 */
			{
				enum { Multidrop_enable = 0x1 };
			};

			struct Tx_b8   : Bitfield<2, 1> { };  /* the ninth bit for RS-485 */
		};

		struct Sr1 : Register<0x94, 32>
		{
			struct RrDy : Bitfield<9, 1> { };
		};
		...
		char _get_char() const
		{
			return read<Rxd::Rx_data>();
		}

		void _init(Driver_settings &settings)
		{
	...
	public:
		 ...
		 * \param  claock  base clock speed
		 * \param  baud    desired baud rate
		 */
		Imx_uart(Platform::Device &device, Driver_settings &settings) :
			Mmio { device }
		{
			_init(settings);
		}
		...
		/**
		 * send character 'c' through the UART
		 */
		...
		{
			return read<Sr1::RrDy>();
		}

		/**
		 * receive character from the UART
		 */
		char get_char()
		{
			return _get_char();
		...
		void enable_irq()
		{
			write<Cr1>(read<Cr1>() | Cr1::Ir_en::bits(1));
		}

		void disable_irq()
		{
			write<Cr1>(read<Cr1>() & Cr1::Ir_en::bits(0));
		}
